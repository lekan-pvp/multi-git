...
func ConfigureGit() (err error) {
	err = exec.Command("git", "config", "--global", "user.email", "lekan.lekan@gmail.com").Run()
	if err != nil {
		return
	}

	err = exec.Command("git", "config", "--global", "user.name", "lekan").Run()
	return
}
...
func RunMultiGit(command string, ignoreErrors bool, mgRoot string, mgRepos string) (output string, err error) {
	...
	if len(out) == 0 {
		err = errors.New("mg is not in PATH")
		return
	}
